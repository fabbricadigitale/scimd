...
func TestDeleteResource(t *testing.T) {
	...
	err := delete.Resource(adapter, resTypeRepo.Get("User"), notExistingID)
	...
	err = delete.Resource(adapter, resTypeRepo.Get("User"), id)
	require.Nil(t, err)

	// checks the successfully deletion
	retRes, err := adapter.Get(resTypeRepo.Get("User"), id, "", nil)
